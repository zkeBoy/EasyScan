...
- (void)drawRect:(CGRect)rect {
    ...
    for (int i=0; i<sectionsNum ; i++) {
        ...
        CGContextAddArc(context, self.center.x, self.center.y-kStatusBarHeight-57, sectionRadius - 5*(sectionsNum - i - 1), 0, 2*M_PI, 0); //添加一个圆
        ...
        for (int index=0; index<pointsNum; index++) {
            if (self.dataSource && [self.dataSource respondsToSelector:@selector(radarView:viewForIndex:)]) {
                if (self.dataSource && [self.dataSource respondsToSelector:@selector(radarView:positionForIndex:)]) {
                    ...
                    pointView.center = CGPointMake(self.center.x + posDistance*sin(DEGREES_TO_RADIANS(posDirection)), self.center.y - kStatusBarHeight - 57 + posDistance*cos(DEGREES_TO_RADIANS(posDirection)));
